 ...
 * @author geraldo
 */
public class OportunidadeCOAP extends javax.swing.JPanel {

    SessionFacadeImpl session = new SessionFacadeImpl();

    /** Creates new form Oportunidade */
    public OportunidadeCOAP() {
        initComponents();
...
/////        cbAgravo.addItem("TÉTANO NEONATAL");
    ...
    private void cbRegionalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbRegionalActionPerformed
        ComboBoxModel modelo;
        modelo = new DefaultComboBoxModel(this.session.retornaMunicipiosCOAP(this.cbDesagregacao.getSelectedIndex(),this.cbUf.getSelectedItem().toString(), this.cbRegional.getSelectedItem().toString()));
...
//        session.setTemListagem(cbGerarListagem.isSelected());
        ...
        session.setRelatorio("OportunidadeCOAP");
        session.execute();
